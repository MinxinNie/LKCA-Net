...
def validation(model, validloader, classes=4):
    # mask_pred, mask_gt = [], []
    model.eval()
    running_metric = 0.
    with torch.no_grad():
        for sampled_batch in validloader:
            ...
            running_metric += calculate_metric_list_percase(pred_batch_np, label_batch_np, classes) * bsz
...
def trainer_ACDC(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss = 0.0
            lc1, lc2 = 0.3, 0.6  # 0.3, 0.7


            loss_ce = ce_loss(out, label_batch[:].long())
            loss_dice = dice_loss(out, label_batch, softmax=True)
