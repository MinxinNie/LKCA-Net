...
from backbone.decoders import CASCADE_Add, CASCADE_Cat
from backbone.maxxvit import maxvit_tiny_rw_224 as maxvit_tiny_rw_224_4out
from backbone.maxxvit import maxvit_rmlp_tiny_rw_256 as maxvit_rmlp_tiny_rw_256_4out
from backbone.maxxvit import maxxvit_rmlp_small_rw_256 as maxxvit_rmlp_small_rw_256_4out
from backbone.maxxvit import maxvit_rmlp_small_rw_224 as maxvit_rmlp_small_rw_224_4out
...
class MaxViT4Out_Small(nn.Module):
    def __init__(self, n_class=1, img_size=224, pretrain=True):
        ...
        if img_size == 224:
            self.backbone = maxvit_rmlp_small_rw_224_4out()  # [64, 128, 320, 512]
            if pretrain:
                print('Loading:', '/home/users/nieminxin/LKCA-NET/backbone/pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
                state_dict = torch.load('./backbone/pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
        elif (img_size == 256):
            self.backbone = maxxvit_rmlp_small_rw_256_4out()
            print('Loading:', './pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
            state_dict = torch.load('./backbone/pretrained_pth/maxvit/maxxvit_rmlp_small_rw_256_sw-37e217ff.pth')
        else:
            sys.exit(str(img_size) + " is not a valid image size! Currently supported image sizes are 224 and 256.")
...
class MERIT_Parallel_Small(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256, 256), img_size_s2=(224, 224), decoder_aggregation='additive',
