...
parser.add_argument('--volume_path', type=str,
                    default='/home/users/nieminxin/Synapse', help='root dir for data')
...
def inference(args, model, test_save_path=None):
    ...
    if args.dataset == 'Synapse':
        db_test = dataset_config[args.dataset]['Dataset'](base_dir=join(args.volume_path, "test_vol_h5"),
                                                          split="test_vol", list_dir=args.list_dir)
    elif args.dataset == 'ACDC':
        db_test = dataset_config[args.dataset]['Dataset'](base_dir=args.volume_path, split="test",
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        metric_i = test_single_volume(image, label, model, classes=args.num_classes,
                                      ...
                                      z_spacing=dataset_config[args.dataset]['z_spacing'])
        metric_list += np.array(metric_i)
        args.logger.info('idx %d case %s mean_dice %f mean_hd95 %f, mean_jacard %f mean_asd %f' % (
    ...
    for i in range(1, args.num_classes):
        args.logger.info('Mean class %d mean_dice %f mean_hd95 %f mean_jacard %f mean_asd %f' % (i, metric_list[i - 1][0], metric_list[i - 1][1],metric_list[i - 1][2], metric_list[i - 1][3]))
    performance = np.mean(metric_list, axis=0)[0]
    mean_hd95 = np.mean(metric_list, axis=0)[1]
    mean_jacard = np.mean(metric_list, axis=0)[2]
    mean_asd = np.mean(metric_list, axis=0)[3]
    args.logger.info('Testing performance in best val model: mean_dice : %f mean_hd95 : %f mean_jacard: %f  mean_asd:%f' % (performance, mean_hd95, mean_jacard, mean_asd))
...
if __name__ == "__main__":
    ...
    msg = net.load_state_dict(torch.load(config.PRETRAIN_CKPT))
